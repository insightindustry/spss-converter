...
.. note::
  ...
  Unit tests for the **SPSS Converter** are written using `pytest`_ and
...
**SPSS Converter**, we decided to focus on practicality. That means:
  ...
  * **Coverage matters...kind of.** We have documented the primary intended
    behavior of every function in the **SPSS Converter** library, and the
...
Each individual test module (e.g. ``test_read.py``) corresponds to a
conceptual grouping of functionality. For example:

* ``test_read.py`` tests functions that de-serialize data from SPSS files, as defined in
  ``spss_converter/read.py``
...
The **SPSS Converter** does not use any custom command-line options in its
test suite.
...
Because the **SPSS Converter** has a very simple test suite, we have not
...
.. note::
  ...
  Because of the simplicity of the **SPSS Converter**, the test suite does
...
.. note::
  ...
  The **SPSS Converter** test suite does support incremental testing using,
